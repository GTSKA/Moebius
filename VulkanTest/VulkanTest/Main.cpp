...
struct Vertex
{
	...
	Vertex(Vector3 _pos, Vector3 _color, Vector2 _texCoord)
	{
		pos = _pos;
		color = _color;
		texCoord = _texCoord;
...
private:
	...
	void initVulkan()
	{
		...
		createDepthResources();//Depth Buffering tutorial
		createFramebuffers();//Drawing tutorial
		createTextureImage();//Texture mapping tutorial
		createTextureImageView();//Texture mapping tutorial : image view and sampler
		createTextureSampler();//Texture mapping tutorial : image view and sampler
	...
	{
		int texWidth, texHeight, texChannels, bpp;
		char* bufferTGA = LoadTGA("../Textures/Rock.tga", &texWidth, &texHeight, &bpp);
		...
		if (bpp == 24)
		{
			char* bufferTGA2 = new char[texWidth*texHeight * 4];
			...
			memcpy(data, bufferTGA2, static_cast<size_t>(imageSize));
			delete[] bufferTGA2;
		}
		else if (bpp == 32)
		{
			memcpy(data, bufferTGA, static_cast<size_t>(imageSize));
			format = VK_FORMAT_R8G8B8A8_UNORM;
		}
		else 
		...
		createImage(texWidth, texHeight, format, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT |
	...
	void createTextureSampler()
	{
		...
		*/
		...
		samplerInfo.unnormalizedCoordinates = VK_FALSE;//if it is VK_FALSE, then the texels are addressed using the [0, 1) else within the [0, texWidth) and [0, texHeight)

		samplerInfo.compareEnable = VK_FALSE;
		samplerInfo.compareOp = VK_COMPARE_OP_ALWAYS;
		//if comaprision function is enabled, then texels will first be compared to a value, and the result of that comparison is used in filtering operations
		...
		VkImageLayout oldLayout, VkImageLayout newLayout)
	{
		...
		vkCmdPipelineBarrier(commandBuffer,
			...
			0, //is either 0 or VK_DEPENDENCY_BY_REGION_BIT, which turns the barrier into a per-region
			//the last three pairs of parameters reference arrays of pipeline barriers
	...
	bool isDeviceSuitable(VkPhysicalDevice device)
	{
		...
		return deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU && deviceFeatures.geometryShader && indices.isComplete() && extensionsSupported && swapChainAdequate && supportedFeatures.samplerAnisotropy;
	}
	...
	void updateUniformBuffer(uint32_t currentImage)
	{
		...
		ubo.world.SetRotationZ(time*M_PI*0.5f);
	...
	const std::vector<Vertex> vertices = {
		Vertex(Vector3(-0.5f, -0.5f, 0.0f), Vector3(1.0f, 1.0f, 0.0f), Vector2(1.0f,0.0f)),
		Vertex(Vector3(0.5f, -0.5f, 0.0f), Vector3(0.0f, 1.0f, 1.0f), Vector2(0.0f,0.0f)),
		Vertex(Vector3(0.5f, 0.5f, 0.0f), Vector3(1.0f, 0.0f, 1.0f), Vector2(0.0f,1.0f)),
		Vertex(Vector3(-0.5f, 0.5f, 0.0f), Vector3(1.0f, 1.0f, 1.0f), Vector2(1.0f,1.0f)),

		Vertex(Vector3(-0.5f, -0.5f, -0.5f), Vector3(1.0f, 1.0f, 0.0f), Vector2(1.0f,0.0f)),
		Vertex(Vector3(0.5f, -0.5f, -0.5f), Vector3(0.0f, 1.0f, 1.0f), Vector2(0.0f,0.0f)),
