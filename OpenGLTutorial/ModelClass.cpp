...
bool ModelClass::Initialize(OpenGLClass* OpenGL, char* textureFilename, unsigned int textureUnit, bool wrap,int pos)
{
	bool result;


	// Initialize the vertex and index buffer that hold the geometry for the triangle.
	result = InitializeBuffers(OpenGL,pos);
	if (!result)
	{
		return false;
	}
...
bool ModelClass::InitializeBuffers(OpenGLClass* OpenGL, int pos)
{
	VertexType* vertices;
	...
	if (pos)
	{
		vertices[0].x = -5.0f;  // Position.
		vertices[0].y = -1.0f;
		vertices[0].z = 0.0f;

		vertices[1].x = -4.0f;  // Position.
		vertices[1].y = 1.0f;
		vertices[1].z = 0.0f;

		vertices[2].x = -3.0f;  // Position.
		vertices[2].y = -1.0f;
		vertices[2].z = 0.0f;
	}
...
unsigned int ModelClass::getVertexArray()
{
	return m_vertexBufferId;
}
unsigned int ModelClass::getIndexArray()
{
