...
Camera::Camera()
{
	m_positionInfo = Vector3(0.0, 5.0, 25.0);
...
void Camera::Init(float width, float height)
{
	...
	float l_near = 0.1f;
	float l_far = 500;
	m_projectionMatrix.SetPerspective(l_fov, width / height, l_near, l_far);//This will change later, but by start is like this
}

void Camera::Init(float i_fov, float i_near, float i_far, float i_speed, float width, float height)
{
	m_worldMatrix.SetIdentity();
	m_viewMatrix.SetIdentity();
	m_projectionMatrix.SetIdentity();
	m_projectionMatrix.SetPerspective(i_fov, width / height, i_near, i_far);//This will change later, but by start is like this
	m_Speed = i_speed;
}
